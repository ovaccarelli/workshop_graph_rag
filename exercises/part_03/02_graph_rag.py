...
# Construct a graph from the retrieved chunks using the same extraction flow as the previous exercise.
start = time.time()
...
# EXERCISE - Merge graphs:
# Add the graph built from the retrieved chunks into the baseline graph loaded from disk.
baseline_graph.merge(...)
...
# EXERCISE - Limit graph facts:
# Choose the maximum number of scored graph edges to keep for the answer prompt.
max_results = ...
...
# EXERCISE - Build the final prompt:
# Fill in the user question, selected graph facts, and retrieved document context.
prompt = f"""
Question:
{...}
